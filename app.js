...
async function hargaShowResult() {
  ...
  // supaya "Asia Singapore Malaysia" dengan paket berbeda semua muncul
...
async function loadEsimAccessPrice(countryDisplay, day, kurs, markup, aviroamPartnerEsim) {
  ...
  try {
    ...
    // Prioritaskan paket single country
    ...
    // === FILTER LOGIC ===
    if (selDay) {
      packages = packages.filter(p => parseInt(p.duration || p.day || 0) === selDay);
    }
    if (selPkg) {
    ...
    packages.forEach(pkg => {
      const d = parseInt(pkg.duration || pkg.day || 0);
      ...
      dayPkgs.forEach(pkg => {
        ...
        html += `<div style="border:${border};border-radius:8px;padding:8px 10px;margin-bottom:7px;background:${bg};">
          <div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;margin-bottom:2px;">
            <span style="font-size:11px;font-weight:600;color:var(--text);">${pkgName}</span>
            ${cheapBadge}
          </div>
...
async function loadIroamlyPrice(country, day, kurs, markup, aviroamPartnerEsim) {
  ...
  try {
    ...
    // === FILTER LOGIC ===
    if (selDay) {
      allPkgs = allPkgs.filter(p => parseInt(p.duration) === selDay);
    }
    if (selPkg) {
      const selLower = selPkg.toLowerCase();
      const isUnlimited = selLower.includes('unlimited');
    ...
    // Sort by durasi ASC lalu data size
    allPkgs.sort((a, b) => {
...
async function loadEsimCardPrice(country, day, kurs, markup, aviroamPartnerEsim) {
  ...
  try {
    ...
    // Sort by validity ASC lalu price ASC
    filtered.sort((a, b) => {
