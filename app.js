...
async function loadContacts(query) {
  ...
  if (filtered.length > 0) { list.innerHTML = filtered.map(c => `<div class="contact-card" onclick="openContactDetail('${escH(c.noWa)}', '${escH(c.nama)}')"><div class="contact-avatar">${String(c.nama || '?').charAt(0).toUpperCase()}</div><div class="contact-info"><div class="contact-name">${c.nama ? escH(c.nama) : '<span style="color:var(--text-hint);font-style:italic;">Belum ada nama</span>'}</div><div class="contact-wa">📱 ${c.noWa}</div><div class="contact-meta">${c.kota ? '📍 '+escH(c.kota)+' · ' : ''}${c.source || 'WA'}</div></div><div class="contact-actions">${c.totalBooking > 0 ? `<div class="contact-badge">${c.totalBooking}x booking</div>` : ''}<button class="btn-chat" onclick="event.stopPropagation();openNewChatModal('${escH(c.noWa)}','${escH(c.nama)}')"><i class="ti ti-message-circle"></i> Chat</button></div></div>`).join(''); }
